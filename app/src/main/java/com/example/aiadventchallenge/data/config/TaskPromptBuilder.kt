...
object TaskPromptBuilder {
    ...
    private fun getPhaseInstructions(phase: TaskPhase): String = when (phase) {
        ...
        ⚠️ КРИТИЧЕСКИЕ ПРАВИЛА ДЛЯ PLANNING (УНИВЕРСАЛЬНЫЕ):

        1. ЧТО НУЖНО ДЕЛАТЬ В PLANNING:
           - Задавайте вопросы для сбора информации (цель, ограничения, ресурсы)
           - Уточняйте детали задачи итеративно
           - Представляйте краткое резюме собранных требований
           - Спрашивайте подтверждение для перехода в EXECUTION

        2. ЧТО ЗАПРЕЩЕНО ДЕЛАТЬ В PLANNING:
           ❌ Выводить структурированный результат (дни/шаги/элементы с деталями)
           ❌ Включать конкретные значения/числа/параметры без вопросов
           ❌ Составлять детальный план/программу/протокол/режим
           ❌ Решать задачу до EXECUTION

        3. УНИВЕРСАЛЬНЫЕ ПРИМЕРЫ:

        ❌ НЕПРАВИЛЬНЫЕ ОТВЕТЫ В PLANNING:
           - Тренировки: "Пн: Жим 4x8, Вт: Присед 5x5..." ❌ (детальный план)
           - Питание: "Пн: Курица 200г, рис 100г..." ❌ (детальный план)
           - Добавки: "Креатин 5г до еды, протеин 30г после..." ❌ (детальный план)
           - Любая область: решение с конкретными значениями и без вопросов ❌

        ✅ ПРАВИЛЬНЫЕ ОТВЕТЫ В PLANNING:
           - "Какую цель преследуете?" ✅ (вопрос)
           - "Сколько раз в неделю можете заниматься?" ✅ (вопрос)
           - "Есть ли ограничения по здоровью?" ✅ (вопрос)
           - "Понял. Цель: набор массы, 3-4 раза в неделю. Составить программу?" ✅ (резюме + вопрос)

        4. ПЕРЕХОД PLANNING → EXECUTION:
           - ТОЛЬКО когда представлено резюме требований
           - И пользователь ЯВНО подтвердил: "да", "составь", "создай"
