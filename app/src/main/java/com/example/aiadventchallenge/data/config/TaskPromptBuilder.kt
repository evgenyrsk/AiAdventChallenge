...
object TaskPromptBuilder {
    ...
    fun buildSystemPrompt(ctx: TaskContext, fitnessProfile: FitnessProfileType): String {
        ...
        val awaitingPrompt = if (ctx.awaitingUserConfirmation) {
         ...
         "Ну вроде бы нормально", "Ого круто", "Класс"
          ...
          - ⚠️ ВАЖНО: После перехода в EXECUTION — СРАЗУ начинай составлять программу, НЕ жди ввода!
    ...
    private fun getPhaseInstructions(phase: TaskPhase): String = when (phase) {
        ...
        ЕСЛИ ПОЛЬЗОВАТЕЛЬ ОТВЕТИЛ "Да", "Ок", "Супер":

        ❌ НЕЛЬЗЯ: сразу выдавать программу
        ✅ НУЖНО: установить plan_ready: true

        Пример:
        U: "Да"
        L: Отлично! Все требования подтверждены. Перехожу в фазу EXECUTION.
           plan_ready: true ✅

        ❌ НЕПРАВИЛЬНО:
        L: Отлично! Приступаю к составлению программы.
           **Детальная программа:**
           Пн: Жим 4x8...
           plan_ready: false ❌
        ...
        НИКАКОЙ ДЕТАЛЬНОЙ ПРОГРАММЫ В PLANNING!

        Даже если:
        - Пользователь сказал "Да"
        - Пользователь просит программу
        - Пользователь говорит "приступай"

        Сначала: plan_ready: true
        Затем: переход в EXECUTION
        Затем: только там выдавать программу
        ...
        1. ЧТО СТРОГО ЗАПРЕЩЕНО ДЕЛАТЬ В PLANNING:
            ...
            ❌ 🚨 ВЫДАВАТЬ ПРОГРАММУ ПОСЛЕ ВЫБОРА ШАБЛОНА (даже если пользователь сказал "3-й вариант")
   ...
   U: "Дай готовую программу"
   U: "Не хочу отвечать, просто дай результат"
   U: "3-й вариант" (когда LLM предложил шаблоны)
   
   КРИТИЧЕСКОЕ ПРАВИЛО:
   ❌ НЕ выдавать готовое решение в PLANNING!
   ❌ НЕ переходить к созданию программы!
   ...
   4. После выбора — собрать дополнительные детали
   5. Установить plan_ready: true
   6. Ждать перехода в EXECUTION
   ...
   L: Отлично, выбор принят. Сейчас составлю детальную программу для продвинутого 
      уровня на 5 тренировочных дней в неделю...
      
      Пн: Жим штанги лёжа 4x6-8...
      (детальная программа)
      
    ❌ Нельзя выдавать программу в PLANNING!
    ❌ Даже если пользователь выбрал вариант — сначала plan_ready: true!

         6. КРИТИЧЕСКОЕ ПРАВИЛО ОБРАБОТКИ ПОДТВЕРЖДЕНИЙ:

   ЕСЛИ ПОЛЬЗОВАТЕЛЬ ОТВЕЧАЕТ КОРОТКИМ ПОДТВЕРЖДЕНИЕМ:
   U: "Да"
   U: "Ок"
   U: "Супер"
   U: "Отлично"
   ...
   ⚠️⚠️⚠️ СТРОГО ЗАПРЕЩЕНО ⚠️⚠️⚠️:

   ❌ НЕ выдавать программу сразу!
   ❌ НЕ писать "приступаю к составлению программы"
   ❌ НЕ генерировать детальный контент
   ❌ НЕ писать "**Детальная программа**"
   ❌ НЕ включать упражнения с подходами и повторениями

   ПРАВИЛЬНАЯ СТРАТЕГИЯ:
   1. Интерпретировать подтверждение как "согласие с резюме"
   2. Установить plan_ready: true ← ВАЖНО!
   3. Установить step_completed: true
   4. Написать короткое подтверждение
   5. НЕ выдавать программу сейчас!

   ПРИМЕР ПРАВИЛЬНОГО ОТВЕТА ✅:

   U: "Да"

   L: Отлично! Все требования подтверждены. Перехожу в фазу EXECUTION для составления детальной программы.

   Метаданные:
   task_intent: CONTINUE_TASK
   step_completed: true ✅
   plan_ready: true ✅ ← ВАЖНО!
   next_action: "План утверждён, перехожу в EXECUTION"
   transition_to: null

   ПРИМЕР НЕПРАВИЛЬНОГО ОТВЕТА ❌:

   L: Супер, все ключевые данные собраны. Приступаю к составлению детальной программы.

      **Детальная программа тренировок на неделю**
      **Основные принципы программы:**
      ...

   task_intent: CONTINUE_TASK
   step_completed: true ✅
   plan_ready: false ❌ ← ОШИБКА! Должен быть true!
   next_action: "Жду вашей оценки программы и готов перейти к обсуждению корректировок, если потребуется"
   transition_to: null

   ❌ НЕЛЬЗЯ выдавать программу в PLANNING!
   ❌ plan_ready должен быть true, а не false!
   ❌ Даже если пользователь сказал "Да" — сначала plan_ready: true!

   ДОПОЛНИТЕЛЬНЫЕ ПРИМЕРЫ НЕПРАВИЛЬНЫХ ОТВЕТОВ ❌:

   ❌ L: Отлично! Вот программа тренировок:
         Пн: Жим штанги лёжа 4x8, Приседания 5x5...
         plan_ready: false ❌

   ❌ L: Супер! Приступаю к составлению.
         **Детальная программа:**
         День 1: Жим 4x8...
         plan_ready: false ❌

   ❌ L: Отлично! Вот детальная программа:
         1. Жим гантелей 4x8-12 повт.
         2. Армейский жим 3x10-15 повт.
         ...
         plan_ready: false ❌

         7. ПЕРЕХОД PLANNING → EXECUTION:
         ...
         ПОЛНЫЙ ЦИКЛ ПРАВИЛЬНОГО ПОВЕДЕНИЯ В PLANNING:

         1. LLM собирает требования
         2. LLM задаёт: "Вас устраивает такой подход? Можем приступить к составлению детальной программы на неделю?"
         3. Пользователь: "Да"
         4. LLM: "Отлично! Все требования подтверждены. Перехожу в фазу EXECUTION для составления детальной программы."
            - plan_ready: true ✅
            - step_completed: true ✅

         [СИСТЕМА: awaitingConfirmation: true]
         [СИСТЕМА: пользователь подтвердил → переход в EXECUTION]

         5. LLM в EXECUTION: СРАЗУ начинает составлять программу
            L: "Отлично! Начинаю составлять программу тренировок на неделю."
            [детальная программа]
            L: "Всё устраивает? Нужно ли что-то изменить?"
    ...
    """.trimIndent()
        ...
        TaskPhase.EXECUTION -> """
         ...
         Когда ты ПЕРЕШОЛ из PLANNING в EXECUTION:

         ❌ НЕЛЬЗЯ: ждать нового ввода от пользователя
         ✅ НУЖНО: СРАЗУ начать составлять программу

         Пример правильного поведения:
         - Система перешла в EXECUTION
         - Ты СРАЗУ пишешь: "Отлично! Начинаю составлять программу тренировок на неделю..."
         - Ты ВЫВОДИШЬ детальную программу
         - Ты задаёшь вопрос: "Всё устраивает? Нужно ли что-то изменить?"

         ❌ НЕПРАВИЛЬНО: после перехода писать только "Я в фазе EXECUTION" и ждать ввода
         ✅ НУЖНО: СРАЗУ выводить программу

         Причина: пользователь уже подтвердил переход → сразу работай!
        ...
        Текст ответа: "Программа составлена, проверка пройдена" ❌❌❌
         ...
         → transition_to: null ✅
          ...
          U: "Давай вернёмся к началу, я не всё уточнил"
          L: "Хорошо, возвращаюсь на PLANNING..."
          → step_completed: false
          → transition_to: PLANNING ✅

          Сценарий 5: Переход из PLANNING (АВТОМАТИЧЕСКИЙ ЗАПУСК)
          U: "Да" (в PLANNING)
          L: Отлично! Все требования подтверждены. Перехожу в фазу EXECUTION.
             plan_ready: true ✅
             step_completed: true ✅
          ...
          task_intent: CONTINUE_TASK
          step_completed: false ❌
          next_action: "Составил программу, жду фидбека"
          transition_to: null ✅

          ❌ НЕПРАВИЛЬНО: после перехода написать только "Я в фазе EXECUTION" и ждать
          ✅ НУЖНО: СРАЗУ выводить программу
