...
Этот файл предназначен для фиксации сравнения:

- `PLAIN_LLM`
- `RAG_BASIC` (retrieval only)
- heuristic rerank
- model rerank
- `RAG_ENHANCED` (`THRESHOLD_PLUS_MODEL_RERANK`)

по 10 контрольным вопросам.
...
- heuristic rerank-вызов
- model rerank-вызов
- `RAG_ENHANCED`-вызов с rewrite + threshold + self-hosted model rerank через `Document Index Server`
